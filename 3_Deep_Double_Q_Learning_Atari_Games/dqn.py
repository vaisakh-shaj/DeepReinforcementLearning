...
def learn(env,
    ...
    # Q values [NOTE] : TODO Epsilon greedy exploration
    q_next = q_func(obs_tp1_float,num_actions, scope="target_q_func", reuse=False)
    q_current = q_func(obs_t_float,num_actions, scope="q_func")

    if not ddqn:
        q_current_label = rew_t_ph + (1-done_mask_ph)*gamma * tf.reduce_max(q_next, axis=1) # [NOTE] : max dont omit, 
                                                               # this is where equation is different from actor-critic update
    else:
    ...
    act=tf.argmax(q_current,axis=1)
    ...
    if not(os.path.exists(logdir)):
        os.makedirs(logdir)
    logz.configure_output_dir(os.path.join(logdir,'%d'%0))
    ...
    for t in itertools.count():
        ...
        # which action to take can be based on epsilon greedy exploration
        
        if not model_initialized or random.random()<exploration.value(t):
            action=random.randint(0, num_actions-1) #[NOTE] : num_actions-1
        else:
            obs=replay_buffer.encode_recent_observation()
            action = session.run(act,feed_dict={obs_t_ph:[obs]}) 
            #print("============hello===========",action,obs.shape)  
        next_obs, reward, done, info = env.step(action) #[NOTE] : How we take care of done
        if done==True:
            last_obs=env.reset() # [NOTE]: storing here to explicitly feed to q_func 
                                  #unlike env.step where we need not explicittly feed
        else:
            last_obs=next_obs
        ...
        if (t > learning_starts and
            ...
            session.run(update_target_fn)
            #3c
            error= session.run([total_error,train_fn],feed_dict={obs_t_ph:obs_batch,act_t_ph:act_batch,
                     rew_t_ph:rew_batch,obs_tp1_ph:next_obs_batch, done_mask_ph:done_mask,
                     learning_rate:optimizer_spec.lr_schedule.value(t)})
            
            #3d
            '''
            if(t%target_update_freq==0):
                session.run(update_target_fn)    [NOTE]: This sought of an update is wrong and wont converge
                num_param_updates+=1
            '''
